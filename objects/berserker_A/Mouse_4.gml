if instance_exists(selected)&& !instance_exists(art_box) {
	if instance_exists(desc_hit){ //Hit move
		if global.hpA <= round(12*global.dmgbuff*global.art1buff) && global.art1 = 1 {
	...
	if instance_exists(desc_measure_flare){ //Measure Flare move
		if global.hpA <= round(6*global.dmgbuff*global.art1buff) && global.art1 = 1 {
		...
		global.cd2 = 2;
		instance_create_depth(x-8,y+sprite_height+16,-10,hpA)
	...
	if instance_exists(desc_melting_heat){ //Melting Heat move
		global.dmgbuff = 1;
		instance_create_depth(sprite_width/2,sprite_height/2,-12,dmgdeal)
		instance_create_depth(0,0,-12,ans_hit)
		...
		global.selfdmg = round((global.hpA/2*3) / (global.maxhull/100))
		global.hull -= round(global.hpA/2 * 3);
		global.hp -= round(global.hpA/2);
		global.hpA = round(global.hpA/2);
	}
